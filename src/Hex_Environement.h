...
public:
    ...
    virtual void otherPlayerMove(int row, int col) {
        //std::cerr << "L'autre joueur a joué le coup : " << row << " " << col << std::endl;
    };
};
...
public:
    ...
    std::tuple<unsigned int, unsigned int> getLastMove() const {
        if(historique_coups.size() == 0) return {-1, -1};
        return historique_coups.back();
    }
...
inline std::tuple<int, int>  Humain::getMove(Hex_Environement& hex) {
    ...
    while(true) {
        if(hex.isWithGUI()) {
            ...
            if (!(std::cin >> row >> col)) {
                if (std::cin.eof()) {
                    std::cerr << "End of input (pipe closed)." << std::endl;
                    _exit(1);
                }
            ...
            if (hex.isValidMove(row, col)) {
                ...
                std::cerr << "Invalid move: position (" << row << "," << col << ") is not allowed." << std::endl;
            }
        }
    }
    return {row, col};
